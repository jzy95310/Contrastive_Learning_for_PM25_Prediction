...
def run_supervised_SimCLR(holdout, requires_meteo=False, train_stations=-1, lr=5e-7):
    ...
    ssl_path = '../../model_checkpoint/encoder_params_resnet50_regular_Beijing_SimCLR.pkl'
...
def cli_main():
    ...
    lrs_no_meteo = [2e-6, 1e-6, 8e-7, 5e-7, 4e-7, 2e-7]   # ResNet50
    lrs_meteo = []   # ResNet50

    for i in range(len(stations_num)):
        rmse, spatial_r, spatial_rmse = [], [], []
        for holdout in holdout_stations:
            res = run_supervised_SimCLR(holdout=holdout, requires_meteo=False, train_stations=stations_num[i], lr=lrs_no_meteo[i])
        ...
        result_path = './model_results/results_SimCLR_regular.pkl'
