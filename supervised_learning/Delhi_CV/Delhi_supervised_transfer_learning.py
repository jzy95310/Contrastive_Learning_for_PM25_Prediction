...
def run_supervised_transfer_learning(holdout, requires_meteo=False, train_stations=-1, lr=5e-8):
...
def cli_main():
    ...
    lrs_no_meteo = [1e-7, 4e-8, 3.5e-8, 3e-8, 1.5e-8, 1e-8]  # ResNet50
    lrs_meteo = []  # ResNet50
    
    for i in range(len(stations_num)):
        rmse, spatial_r, spatial_rmse = [], [], []
        for holdout in holdout_stations:
            res = run_supervised_transfer_learning(holdout=holdout, requires_meteo=False, train_stations=stations_num[i], lr=lrs_no_meteo[i])
