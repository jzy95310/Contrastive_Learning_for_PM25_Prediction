...
def run_supervised_transfer_learning(requires_meteo=False, train_stations=-1, lr=5e-8):
    ...
    if requires_meteo:
        model = ResNet50_ImageNet_pretrained_joint_meteo(transformed_meteo_dim).to(device)
    else:
        model = ResNet50_ImageNet_pretrained_no_meteo().to(device)
...
def cli_main():
    ...
    lrs_no_meteo = [1e-4, 1e-5, 1e-5, 1e-5, 5e-6, 1e-6]
    lrs_meteo = [1e-6, 1e-7, 1e-7, 1e-7, 5e-8, 1e-8]
